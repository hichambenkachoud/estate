...
class LoginFormAuthenticator extends AbstractFormLoginAuthenticator
{
    ...
    {
        return 'front_security_login' === $request->attributes->get('_route')
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        $user = $this->entityManager->getRepository(Members::class)->loadMemberByUsername($credentials['username']);

        if (!$user) {
            // fail authentication with a custom error
            throw new CustomUserMessageAuthenticationException($this->trans->trans('front.credentials.invalid'));
        }
        return $user;
    }
    ...
    {
        if ($auth = $this->passwordEncoder->isPasswordValid($user, $credentials['password'])){
            return $auth;
        }else{
            throw new CustomUserMessageAuthenticationException($this->trans->trans('front.credentials.invalid'));
        }
